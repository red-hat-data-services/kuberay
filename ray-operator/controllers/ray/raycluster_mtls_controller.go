...
const (
	...
	RayClusterMTLSPeriodicCheckDuration  = 1 * time.Minute
)
...
func (r *RayClusterMTLSController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if instance.Annotations == nil || instance.Annotations[utils.EnableSecureTrustedNetworkAnnotationKey] != "true" {
		logger.Info("MTLS is disabled for RayCluster, skipping reconciliation", "rayCluster", instance.Name)
		return ctrl.Result{}, nil
	}

	logger.Info("MTLS is enabled for RayCluster", "rayCluster", instance.Name)

	// Reconcile self-signed issuer (needed to bootstrap the CA)
	if err := r.reconcileSelfSignedIssuer(ctx, instance); err != nil {
		logger.Error(err, "Failed to reconcile self-signed issuer")
...
func (r *RayClusterMTLSController) cleanupMTLSResources(ctx context.Context, namespace, clusterName string) (ctrl.Result, error) {
	...
	caCertName := fmt.Sprintf("%s-%s", caCertificateName, clusterName)
	if err := r.deleteCertificate(ctx, namespace, caCertName); err != nil {
		logger.Error(err, "Failed to delete CA certificate", "certificate", caCertName)
		errors = append(errors, err)
	}
...
// reconcileSelfSignedIssuer reconciles the self-signed issuer for bootstrapping the CA
func (r *RayClusterMTLSController) reconcileSelfSignedIssuer(ctx context.Context, instance *rayv1.RayCluster) error {
	...
	issuerName := fmt.Sprintf("%s-%s", raySelfSignedIssuerName, instance.Name)
	err := r.Get(ctx, client.ObjectKey{Name: issuerName, Namespace: instance.Namespace}, selfSignedIssuer)

	if err == nil {
		// Self-signed issuer exists, do nothing
		return nil
	} else if errors.IsNotFound(err) {
		// Self-signed issuer does not exist, create it
		logger.Info("Creating self-signed issuer for RayCluster", "rayCluster", instance.Name)
		return r.createSelfSignedIssuer(ctx, instance)
	}
	return err
}
...
// createSelfSignedIssuer creates a self-signed issuer for bootstrapping the CA
func (r *RayClusterMTLSController) createSelfSignedIssuer(ctx context.Context, instance *rayv1.RayCluster) error {
	logger := log.FromContext(ctx)
	...
	issuer := &certmanagerv1.Issuer{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				"app.kubernetes.io/component": "selfsigned-issuer",
	...
	if err := r.Create(ctx, issuer); err != nil {
		logger.Error(err, "Failed to create self-signed issuer")
		return err
	}

	logger.Info("Self-signed issuer created successfully", "rayCluster", instance.Name)
	return nil
}
...
func (r *RayClusterMTLSController) createRayHeadCertificate(ctx context.Context, instance *rayv1.RayCluster, podIPs []string) error {
	...
	certificate := &certmanagerv1.Certificate{
		...
		Spec: certmanagerv1.CertificateSpec{
			...
			DNSNames:    dnsNames,
			IPAddresses: normalizeIPs(podIPs), // Always includes 127.0.0.1
...
// GetCAIssuer retrieves the CA issuer from the specified namespace and cluster
func (r *RayClusterMTLSController) GetCAIssuer(ctx context.Context, namespace, clusterName string) (*certmanagerv1.Issuer, error) {
	caIssuer := &certmanagerv1.Issuer{}

	issuerName := fmt.Sprintf("%s-%s", caIssuerName, clusterName)
	err := r.Get(ctx, client.ObjectKey{Name: issuerName, Namespace: namespace}, caIssuer)
	if err != nil {
		return nil, err
	}
