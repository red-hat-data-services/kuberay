...
func InjectAuthSidecar(
	...
	// Determine if sidecar should be injected based on auth mode and cluster annotation
	// Both OAuth and OIDC modes are supported
	shouldInject := ShouldEnableOAuth(cluster, authMode) || ShouldEnableOIDC(cluster, authMode)
